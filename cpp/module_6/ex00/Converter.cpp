...
bool    StringConverter::isChar() {
    ...
    if ((str_.length() == 0) || (str_.length() == 1 && !std::isdigit(str_[0])))
        ...
        if(strtod_val_ < dcasted_char_min || dcasted_char_max < strtod_val_ || isnan(strtod_val_) || isinf(strtod_val_))
...
bool    StringConverter::isInt() {
    ...
    if(strtod_val_ < dcasted_int_min || dcasted_int_max < strtod_val_ || isnan(strtod_val_) || isinf(strtod_val_)) {
...
bool    StringConverter::isinff() {
    ...
    return (isinf(strtod_val_) && *strtod_end_ && (*strtod_end_ == 'f' || *strtod_end_ == 'F') && !*(strtod_end_ + 1));
}

bool    StringConverter::isnanf() {

    return (isnan(strtod_val_) && *strtod_end_ && (*strtod_end_ == 'f' || *strtod_end_ == 'F') && !*(strtod_end_ + 1));
}
...
bool    StringConverter::isFloat() {
    ...
    if ((isinf(fcasted_strtod_abs_val) && !isinf(strtod_abs_val)) || (isnan(fcasted_strtod_abs_val) && !isnan(strtod_abs_val)) || (!isinf(strtod_abs_val) && !isnan(strtod_abs_val) && dcasted_float_max < strtod_abs_val)) {
...
void    StringConverter::setFloatSS() {
    ...
    if (is_invalid_input || is_out_of_range_float)
        ...
        if (str_float_val_.find(kDecimalPoint, kFloatPrompt.length()) == std::string::npos && str_float_val_.find(kExponent, kFloatPrompt.length()) == std::string::npos && !isnan(float_val_) && !isinf(float_val_))
            float_ss_ << ".0f";
        else
            float_ss_ << "f";
    }
...
void    StringConverter::setDoubleSS() {
    ...
    if (is_invalid_input)
        ...
        if (str_double_val_.find(kDecimalPoint, kDoublePrompt.length()) == std::string::npos && str_double_val_.find(kExponent, kDoublePrompt.length()) == std::string::npos && !isnan(double_val_) && !isinf(double_val_))
            double_ss_ << ".0";
    }
}
